...
int main(int argc, char* argv[]){
 ...
 // 	char ch;
	...
	string comparePath = "compare.txt";
	compareFile.open(comparePath);
	string compare_roi;*/
	...
	while ( getline(listFramesFile, frameName) ){
		...
		/*getline(compareFile, compare_roi);
		istringstream compare_ss(compare_roi);
		float xMin_, yMin_, width_, height_;
		char ch_;
		compare_ss >> xMin_;
		compare_ss >> ch_;
		compare_ss >> yMin_;
		...
		if (nFrames == 0) {
			...
			//rectangle(frame, Point(xMin_, yMin_), Point(xMin_ + width_, yMin_ + height_), Scalar(125, 0, 125), 1, 8);
